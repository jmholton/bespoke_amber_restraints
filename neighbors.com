...
set subject_atoms  = `awk '/^ATOM/ || /^HETATM/{print} END{print "extra"}' ${tempfile}both.pdb | wc -l`
...
RADII X $half_radius
DMIN $dmin
FROM ATOM 1 to $subject_atoms
TO   ATOM $subject_atoms to 99999999
END
EOF
...
EOF
    ...
    egrep "^CRYST1" ${tempfile}sparse.pdb | head -n 1 >! neighbors_gathered.pdb
    ...
    cat >> neighbors_gathered.pdb

    set n = `egrep "^ATOM|^HETAT" neighbors_gathered.pdb | wc -l`
    echo "$n atoms is ready in neighbors_gathered.pdb"
